...
def test_contact_admin_post_with_invalid_form(authenticate_user):
    ...
    assert response.status_code == 200
    assert "Mail n&#x27;a pas pu être envoyé, merci d&#x27;essayer de nouveau" in response.content.decode("utf-8")
...
def test_follow_user_with_invalid_form(authenticate_user):
    ...
    templates = [template.name for template in response.templates]
    assert 'blog/follow_user.html' in templates
    assert "Abonnement n&#x27;a pas pu être réalisé, merci d&#x27;essayer de nouveau" in response.content.decode("utf-8")
